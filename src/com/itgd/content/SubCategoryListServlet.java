	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if(request.getParameter("scid")!=null){
				subCategoryId = Integer.parseInt(request.getParameter("scid"));
				if(request.getParameter("p")!=null)
					currentPage = Integer.parseInt(request.getParameter("p"))+1;
				if(currentPage<=1)
					currentPage=1;
			} else {
				//System.out.println("SubCategoryListServle----"+pagePath);
				...
				if(tokens.length>Constants.FOUR) {
					...
					if(tokens.length>=Constants.FOUR) {
						...
						subCategoryId=Integer.parseInt(tempGalleryId);
					}
				} else if(tokens.length==Constants.FOUR) {
					subCategoryId=Integer.parseInt(tokens[Constants.TWO]);
					currentPage=1;
				}
			}
			request.setAttribute("currentSubCategoryListId",subCategoryId);
			request.setAttribute("currentSubCategoryListPageNo",currentPage);

			RequestDispatcher dispatcher = request.getRequestDispatcher(Constants.SUB_CATEGORY_PAGE);
