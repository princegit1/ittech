	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(tokens.length>=Constants.TWO) {
				contentType=tokens[Constants.TWO];
			}
			...
			if(tokens.length>=Constants.FOUR) {
				contentId=Integer.parseInt(tokens[Constants.FOUR]);
			}
			if(tokens.length>Constants.FIVE) {
				slideId=Integer.parseInt(tokens[Constants.FIVE]);
				request.setAttribute("slideId",slideId);
			}
			request.setAttribute("currentPageNo",currentPage);
			request.setAttribute("contentId",contentId);
			request.setAttribute("contentType",contentType);
			RequestDispatcher dispatcher = request.getRequestDispatcher(Constants.COMMENT_PAGE);
