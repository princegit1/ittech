...
public class GalleryServletTech extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public GalleryServletTech() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			galleryLandingId=Integer.parseInt(storyIdUrl[0]);
			 currentPage=Integer.parseInt(dataUrl[dataUrl.length-2]);
			//System.out.println("categoryId>>>>>>>>>"+categoryId+"<<<<<<<<<<<"+currentPage);	
			request.setAttribute("currentGalleryLandingId",galleryLandingId);
			request.setAttribute("currentGalleryLandingPageNo",currentPage);
			request.setAttribute("newPagePath", pagePath);
			RequestDispatcher dispatcher = request.getRequestDispatcher(Constants.GALLERYTECH);
